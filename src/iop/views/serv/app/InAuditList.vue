...
export default {
  name: 'PendingApplicationTable',
  components: { IopDataTable, AppDetailForm },
  data () {
    return {
      table: {
        url: path.serv.app.index,
        query: { status: IOP_SERV_APP_STATUS.INAUDIT },
        rowKey: 'appId',
        columns: appListVo,
        selectable: false
      },
  ...
  methods: {
    async search () {
      try {
        await this.$refs.inAuditList.search()
      } catch (error) {
    ...
    resetSearchForm () {
      this.$refs.inAuditList.resetSearch()
      this.$refs.inAuditList.reload()
    },
