  ...
  >
    <template #form>
      ...
      <a-form-item label="变量个数" v-bind="formItemLayout">
        <a-input v-decorator="['variableNum',{ rules: [{ pattern: new RegExp(/^[1-9]\d{0,8}$/), message: '变量个数只能输入正整数且最多9位' }] }]" />
      </a-form-item>
      <a-form-item label="模板内容" v-bind="formItemLayout">
        <a-textarea placeholder="请输入模板内容" :autosize="{ minRows: 3, maxRows: 6 }" v-decorator="['msgContent', {rules: [{ max: 300, message: '模版内容最多300位' }]}]"/>
      </a-form-item>
      <a-form-item label="备注：" v-bind="formItemLayout">
        <a-textarea placeholder="请输入备注" :autosize="{ minRows: 3, maxRows: 6 }" v-decorator="['remark', {rules: [{ max: 500, message: '模版内容最多500位' }]}]"/>
      </a-form-item>
    </template>
  </iop-data-form>
</template>
