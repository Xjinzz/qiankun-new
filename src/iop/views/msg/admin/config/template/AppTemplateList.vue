<template>
  <div>
    ...
    >
        ...
        <a-row :gutter="{ xs: 8, sm: 16, md: 24, lg: 32 }">
          <a-col :span="8">
            <a-form-item label="模板名称">
              <a-input @pressEnter="search" placeholder="请输入模板名称" v-model="queryParam.msgName" allowClear />
            </a-form-item>
          </a-col>
          <a-col :span="8">
            <a-form-item label="模板编码">
              <a-input @pressEnter="search" placeholder="请输入模板编码" v-model="queryParam.msgCode" allowClear />
            </a-form-item>
          </a-col>
      ...
      <template #tree>
        <app-template-tree ref="templateTree" @onSelect="onTreeSelect" />
      </template>
    </iop-data-table>

    <app-template-form ref="templateForm" :id="id" :parentId="parentId" :mode="formMode" :submitSuccess="reloadNode" />
  </div>
</template>
<script>
import AppTemplateTree from './AppTemplateTree'
...
export default {
  ...
  data () {
    ...
    return {
      ...
      table: {
        /** 列表查询地址 */
        url: path.msg.appTemplate.index,
        /** rowKey，必填 */
        rowKey: 'id',
        /** 列定义 */
  ...
  methods: {
    ...
    async search () {
      try {
        ...
        await this.$refs.templateList.search(this.queryParam)
      } catch (error) {
