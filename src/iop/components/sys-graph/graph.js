...
import d3 from './d3'
var width = 1160, height = 460, color = d3.scale.category20()
//var local = 'http://118.190.199.44:7476'
var localHost = ''
var local = 'http://118.190.199.44:7476'
var svg = null
// 实时获取SVG画布坐标
...
function setMarkers() {
  svg.append('g')
    ...
    .attr('fill', function (d) {
      var typeNum = d.type
      if (typeNum == '2' || typeNum == '3') { //父类关系
        return '#ff5757'
      } else if (typeNum == 'Foreign') { //子类关系
        return '#7e4cef'
      } else if (typeNum == 'Inherit') { //子类关系
        return '#47ef1f'
      }
    })
}
...
function addToolTip(d) {
  var htmlStr = ''
  if (d.hasOwnProperty('sourceId')) {
    if (d.type == 'Investment') {
      if (d.hasOwnProperty('capital')) {
        htmlStr += '<b>' + d.name + '</b></br>' + '控股:' + d.ratio + '</br>' + '资金:' + d.capital + '</br>'
      } else {
        htmlStr += '<b>' + d.name + '</b></br>' + '控股:' + d.ratio + '</br>'
      }
    } else {
      htmlStr += '<b>' + d.name + '</b></br>'
    }
  } else {
    if (d.labels == 'DrmsObject') {  //平台点
      htmlStr += '<b>' + d.name + '</b></br>' + d.code + '</br>'
    } else {
      htmlStr += '<b>' + d.name + '</b></br>'
    }
  }
...
function refresh() {
  force.on('tick', function () { // 对于每一个时间间隔
    // 更新连线坐标·
    if (edges[0] != 'undefined' && edges[0] != '' && edges[0] != null) {
...
export function graphLoad (renderDom, code) {
  svg = d3.select(renderDom)
    .append('svg')
    .attr('id', 'svgGraph')
    .attr('width', width)
    .attr('height', height)
  ...
  var test = 'idl_dwd_enterprise'
  if (code != null) {
    test = code
  }
  d3.json(localHost + '/iop/drms/data-graph/list/' + test, function (error, json) { // 服务器加载知识图谱数据
    if (error) {
